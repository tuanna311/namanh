 ...
 * [namanh_product_showcase]
 */
function namanh_product_showcase_shortcode() {
    $tabs = array(
        array('id' => 50, 'title' => 'VẬT LIỆU NỘI THẤT'),
        array('id' => 49, 'title' => 'SÀN GỖ CÔNG NGHIỆP'),
        array('id' => 52, 'title' => 'PHỤ KIỆN'),
    );

    ob_start();
    ?>
    <div class="namanh-showcase">
        <div class="showcase-inner">
            <div class="showcase-tabs">
                <?php foreach ($tabs as $index => $tab) : ?>
                    <div class="showcase-tab-btn <?php echo ($index === 0) ? 'active' : ''; ?>" 
                         data-target="showcase-panel-<?php echo $tab['id']; ?>">
                        <?php echo $tab['title']; ?>
                    </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="showcase-panels">
                ...
                ?>
                <div id="showcase-panel-<?php echo $tab['id']; ?>" class="showcase-panel <?php echo $active_class; ?>">
                    <div class="showcase-left">
                        <ul class="subcat-list">
                            <?php if (!empty($children)) : ?>
                                <?php foreach ($children as $c_index => $child) : 
                                    $thumb_id = get_term_meta($child->term_id, 'thumbnail_id', true);
                                    $image_url = $thumb_id ? wp_get_attachment_url($thumb_id) : '';
                                    $c_active = ($c_index === 0) ? 'active' : '';
                                    
                                    // Rút gọn tên nếu cần (ví dụ: "Gỗ Plywood Melamine" -> "Melamine")
                                    $short_name = str_replace(array('Gỗ Plywood ', 'Gỗ Polywood '), '', $child->name);
                                ?>
                                <li class="subcat-item <?php echo $c_active; ?>" 
                                    data-image="<?php echo esc_url($image_url); ?>"
                                    data-label="<?php echo esc_attr($short_name); ?>">
                                ...
                                <?php endforeach; ?>
                            <?php else : ?>
                                <li class="subcat-item empty">Chưa có danh mục con</li>
                            <?php endif; ?>
                        </ul>
                    </div>
                    <div class="showcase-right">
                        <?php if (!empty($children)) : ?>
                            <?php foreach ($children as $c_index => $child) : 
                                ...
                                $short_name = str_replace(array('Gỗ Plywood ', 'Gỗ Polywood '), '', $child->name);
                            ?>
                                <div class="image-preview <?php echo $c_active; ?>" data-id="<?php echo $child->term_id; ?>">
                                    <img class="preview-img" src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($child->name); ?>">
                                    <div class="image-label"><?php echo $short_name; ?></div>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
...
add_shortcode('namanh_product_showcase', 'namanh_product_showcase_shortcode');
