				...
				<div class="plugin-card">
					<div class="plugin-card-top">
							...
							<h3>
								<a href="<?php echo esc_url($plugin->permalink); ?>" target="_blank" class="plugin-name">
									...
									<img class="plugin-icon" src="<?php if (isset($plugin->image) && ($plugin->image != '')) {
										...
										echo esc_url(IBTANA_PLUGIN_URI . 'dist/images/ibtana-setting-icon.svg');
									} ?>" />
								</a>
							</h3>
						</div>
						...
						<div class="desc column-description">
							<p>
								<?php echo esc_html($plugin->description, 'ibtana-visual-editor'); ?>
							</p>
	...
	public function ive_plugin_activated($plugin, $network_wide)
	{
		...
		if (is_admin() && current_user_can('install_themes') && (IVE_BASE == $plugin)) {
			...
			if (count(wp_get_themes()) && (gettype(wp_get_themes()) == 'array')) {
				...
				if (str_replace(' ', '', strtolower(wp_get_theme()->get('Author'))) != 'vwthemes' && !isset(wp_get_themes()['sirat'])) {
			...
			if (defined('VW_FREE_THEME')) {
				...
				$themeSlug = strtolower(wp_get_theme()->get('Name'));
				$themeSlug = preg_replace('/\s+/', '', $themeSlug);

				if (str_contains($_SERVER['HTTP_REFERER'], 'themes.php?page=tgmpa-install-plugins')) {
					$yourURL = get_admin_url() . "admin.php?page=". $themeSlug ."-setup";
					echo ("<script>window.location.href=". esc_url($yourURL) ."</script>");
					exit;
				} else {
					wp_redirect('admin.php?page=' . $themeSlug . '-setup');
					exit;
				}
			}
		}
		...
		<script type="text/javascript">
			(function($, window, document) {
				$(document).ready(function() {
					...
					var $wp_get_theme_TextDomain = '<?php echo esc_html($wp_get_theme_TextDomain); ?>';
					var $is_CUSTOM_TEXT_DOMAIN_defined = '<?php echo esc_html($is_CUSTOM_TEXT_DOMAIN_defined); ?>';
					$.post(
						...
						function(data) {
							if (!data.data.is_found) {
								var $ive_active_vw_theme_text_domain = '<?php echo esc_html($ive_active_vw_theme_text_domain); ?>';
								$.post(
									IBTANA_LICENSE_API_ENDPOINT + 'get_client_meta_box_info', {
									...
									function(data) {
										if (!data.data.is_found) {
											$('#ive_go_pro_template_metabox .inside').append(
												...
												<br>
												<a class="ive_go_pro_metabox_a1 button" href="https://www.vwthemes.com/products/wp-theme-bundle?iva_bundle=true" target="_blank">
												Buy Now!
												</a>
												</p>`
											);
								...
								if ($is_CUSTOM_TEXT_DOMAIN_defined === "1") {
									$('#ive_go_pro_template_metabox .inside').append(
										`<p id="ive_go_pro_metabox_p" class="ive_go_pro_metabox_p">
											Get all our <strong>160+ Premium Themes</strong> worth $9440 With Our <strong>WP Theme Bundle</strong> in just <strong>$99.</strong>
											<br>
											<a class="ive_go_pro_metabox_a1 button" href="https://www.vwthemes.com/products/wp-theme-bundle?iva_bundle=true" target="_blank">Buy Now!</a>
										</p>`
									);
									$('#ive_go_pro_template_metabox h2').text(
										'WP Theme Bundle'
	...
	public function ibtana_visual_editor_setup_free_demo()
	{
		...
		if ($post_temp_type	==	'woocommerce') {
			...
			if (!isset($_POST['page_id'])) {
				...
				if (file_exists(get_stylesheet_directory() . '/page-template/ibtana-page-template.php')) {
					update_post_meta($home_id, '_wp_page_template', 'page-template/ibtana-page-template.php');
				}
			} else {
			...
			window.addEventListener('load', function() {
				...
				function ibtana_visual_editor_get_premium_themes() {
					...
					jQuery.ajax({
						...
						data: JSON.stringify({
							"admin_user_ibtana_license_key": '<?php echo esc_html(get_option('vw_pro_theme_key')) ?>',
							"domain": '<?php echo esc_html(site_url()); ?>'
						}),
						dataType: 'json',
...
class Ibtana_Visual_Editor_Menu_Creator extends Ibtana_Visual_Editor_Menu_Class
{
	...
	?>
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				postboxes.add_postbox_toggles('<?php echo esc_html($this->ivehook); ?>');
			});
		</script>
	<?php
	}
	...
	public function ibtana_visual_editor_ive_woocommerce_templates_page()
	{
		wp_enqueue_script('ibtana-admin-wizard-script');
	?>
		<div class="ive-plugin-admin-page">
			...
			<div class="ive-ibtana-wizard-button-wrapper">
				<div class="ibtana-button-wrap">
					<a class="ibtana-free-template-button button button-primary" data-callback="do_next_step" data-step="ive-wizard-first-step" data-template-type="woocommerce">
						<span class="dashicons dashicons-format-image"></span>
						<?php esc_html_e('WooCommerce Templates', 'ibtana-visual-editor'); ?>
					</a>
				</div>
			</div>
			<!-- Main Tabs END -->
	...
	public function ibtana_visual_editor_ive_envato_page()
	{
		...
		if (is_wp_error($response)) {
			?>
			<h4>
				<?php esc_html_e('Enter your add-ons license keys here to receive updates for purchased add-ons.', 'ibtana-visual-editor'); ?>
			</h4>
			<?php
		} else if (($response['response']['code'] === 200) && ($response['response']['message'] === 'OK')) {
			$response = json_decode($response['body']);
